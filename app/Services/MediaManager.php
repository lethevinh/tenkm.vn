...
class MediaManager extends Extension
{
    ...
    public function delete($path)
    {
        ...
        foreach ($paths as $path) {
            ...
            if (is_file($fullPath)) {
                $name = Str::replaceFirst('public/', '', $path);
                $media = Media::where('path_lb', $name)->first();
                if ($media) {
                    $media->delete();
    ...
    {
        $name = Str::replaceFirst('public/', '', $this->path);
        $media = Media::where('path_lb', $name)->first();
        if ($media) {
            $newName = Str::replaceFirst('public/', '', $new);
            $media->update([
                'title_lb' => basename($newName),
                'image_lb' => $newName,
                'path_lb' => $newName
            ]);
    ...
    public function upload($files = [])
    {
        foreach ($files as $file) {
            $path = Str::replaceFirst('public/','', $this->path . '/' . $file->getClientOriginalName()) ;
            $type = $this->detectFileType($file->getClientOriginalName());
            $type = $type === 'video' ? 'video' : 'document';
            $media = Media::create([
                'title_lb' => $file->getClientOriginalName(),
                'image_lb' => $path,
                'type_lb' => $type,
                'status_sl' => 'public',
                'path_lb' => $path
    ...
    public function formatDirectories($dirs = [])
    {
        ...
        $preview = "<a data-href=\"$url\" class='folder-name no-margin margin-0'><span class=\"file-icon text-aqua\"><i class=\"fa fa-folder\"></i></span></a>";
