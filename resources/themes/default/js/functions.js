...
function onScrollSliderParallax() {
	if (!requesting) {
		...
		requestAnimationFrame(function(){
			VITHEME.slider.sliderParallax();
			VITHEME.slider.sliderElementsFade();
		});
	}
	killRequesting();
}
...
(function($){
	...
	VITHEME.initialize = {
		...
		init: function(){
			...
			VITHEME.initialize.responsiveClasses();
			VITHEME.initialize.stickyElements();
			VITHEME.initialize.goToTop();
			VITHEME.initialize.lazyLoad();
			VITHEME.initialize.fullScreen();
			VITHEME.initialize.verticalMiddle();
			VITHEME.initialize.lightbox();
			VITHEME.initialize.resizeVideos();
			VITHEME.initialize.imageFade();
			VITHEME.initialize.pageTransition();
			VITHEME.initialize.dataResponsiveClasses();
			VITHEME.initialize.dataResponsiveHeights();
			VITHEME.initialize.stickFooterOnSmall();
			VITHEME.initialize.stickyFooter();
				...
				$lightboxAjaxEl.magnificPopup({
					...
					callbacks: {
						ajaxContentAdded: function(mfpResponse) {
							VITHEME.widget.loadFlexSlider();
							VITHEME.initialize.resizeVideos();
							VITHEME.widget.masonryThumbs();
						},
						open: function() {
		...
		topScrollOffset: function() {
			...
			if( ( $body.hasClass('device-xl') || $body.hasClass('device-lg') ) && !VITHEME.isMobile.any() ) {
		...
		setFullColumnWidth: function( element ){
			...
			if( element.hasClass('portfolio-full') ) {
				var columns = VITHEME.initialize.defineColumns( element ),
	...
	VITHEME.header = {
		...
		init: function(){
			...
			VITHEME.header.superfish();
			VITHEME.header.menufunctions();
			VITHEME.header.fullWidthMenu();
			VITHEME.header.overlayMenu();
			VITHEME.header.stickyMenu();
			VITHEME.header.stickyPageMenu();
			VITHEME.header.sideHeader();
			VITHEME.header.sidePanel();
			VITHEME.header.onePageScroll();
			VITHEME.header.onepageScroller();
			VITHEME.header.logo();
			VITHEME.header.topsearch();
			VITHEME.header.topcart();
		...
		stickyMenu: function( headerOffset ){
			if ($window.scrollTop() > headerOffset) {
				if( $body.hasClass('device-xl') || $body.hasClass('device-lg') ) {
					...
					VITHEME.header.stickyMenuClass();
				...
				VITHEME.header.removeStickyness();
			}
		},
		...
		removeStickyness: function(){
			if( $header.hasClass('sticky-header') ){
				...
				if( !$headerWrap.hasClass('force-not-dark') ) { $headerWrap.removeClass('not-dark'); }
				VITHEME.slider.swiperSliderMenu();
				VITHEME.slider.revolutionSliderMenu();
			}
			...
			if( $onePageMenuEl.length > 0 ){
				...
				$onePageMenuEl.find('a[data-href]').off( 'click' ).on( 'click', function(){
					...
					if( $( divScrollToAnchor ).length > 0 ) {
						...
						if( !onePageOffset ) {
							var onePageOffsetG = VITHEME.initialize.topScrollOffset();
						} else {
		...
		onepageScroller: function(){
			$onePageMenuEl.find('li').removeClass('current');
			$onePageMenuEl.find('a[data-href="#' + VITHEME.header.onePageCurrentSection() + '"]').parent('li').addClass('current');
		},
	...
	VITHEME.slider = {
		...
		init: function() {
			...
			VITHEME.slider.sliderParallaxDimensions();
			VITHEME.slider.sliderRun();
			VITHEME.slider.sliderParallax();
			VITHEME.slider.sliderElementsFade();
			VITHEME.slider.captionPosition();
		...
		sliderParallax: function(){
			...
			var parallaxOffsetTop = VITHEME.slider.sliderParallaxOffset(),
		...
		swiperSliderMenu: function( onWinLoad ){
			...
			if( $body.hasClass('device-xl') || $body.hasClass('device-lg') || ( $header.hasClass('transparent-header-responsive') && !$body.hasClass('primary-menu-open') ) ) {
				...
				VITHEME.slider.headerSchemeChanger(activeSlide, onWinLoad);
			}
		},
	...
	VITHEME.portfolio = {
		...
		arrange: function(){
			if( $portfolio.length > 0 ) {
				$portfolio.each( function(){
					var element = $(this);
					VITHEME.initialize.setFullColumnWidth( element );
				});
			}
		},
		...
		ajaxload: function(){
			$('.portfolio-ajax .portfolio-item a.center-icon').off( 'click' ).on( 'click', function(e) {
				...
				if( !$(this).parents('.portfolio-item').hasClass('portfolio-active') ) {
					VITHEME.portfolio.loadItem(portPostId, prevPostPortId);
				}
				e.preventDefault();
			});
		},
		...
		newNextPrev: function( portPostId ){
			var portNext = VITHEME.portfolio.getNextItem(portPostId);
			var portPrev = VITHEME.portfolio.getPrevItem(portPostId);
		...
		loadItem: function( portPostId, prevPostPortId, getIt ){
			...
			var portPrev = VITHEME.portfolio.getPrevItem(portPostId);
			if(getIt == false) {
				VITHEME.portfolio.closeItem();
				$portfolioAjaxLoader.fadeIn();
				var portfolioDataLoader = $('#' + portPostId).attr('data-loader');
				$portfolioDetailsContainer.load(portfolioDataLoader, { portid: portPostId, portnext: portNext, portprev: portPrev },
				function(){
					VITHEME.portfolio.initializeAjax(portPostId);
					VITHEME.portfolio.openItem();
		...
		openItem: function(){
			...
			if( noOfImages > 0 ) {
				$portfolioDetails.find('img').on('load', function(){
					noLoaded++;
					var topOffsetScroll = VITHEME.initialize.topScrollOffset();
	...
	VITHEME.widget = {
		...
		init: function(){
			...
			VITHEME.widget.animations();
			VITHEME.widget.youtubeBgVideo();
			VITHEME.widget.tabs();
			VITHEME.widget.tabsJustify();
			VITHEME.widget.tabsResponsive();
			VITHEME.widget.tabsResponsiveResize();
			VITHEME.widget.toggles();
			VITHEME.widget.accordions();
			VITHEME.widget.counter();
			VITHEME.widget.roundedSkill();
			VITHEME.widget.progress();
			VITHEME.widget.twitterFeed();
			VITHEME.widget.flickrFeed();
			VITHEME.widget.instagramPhotos( '5834720953.1677ed0.a0a26ba4c90845f9a844d64c316bf77a', '8e000fefe3024b2ead6a50ff005bf036' );
			VITHEME.widget.dribbbleShots( '012d3d72d12f93e1d41a19195d7da2fc87e6b5afa48a184256e398eb793cfe56' );
			VITHEME.widget.navTree();
			VITHEME.widget.textRotater();
			VITHEME.widget.carousel();
			VITHEME.widget.linkScroll();
			VITHEME.widget.ajaxForm();
			VITHEME.widget.subscription();
			VITHEME.widget.stickySidebar();
			VITHEME.widget.cookieNotify();
			VITHEME.widget.cartQuantity();
			VITHEME.widget.extras();
				...
				$flexSliderEl.each(function() {
					...
					$flexsSlider.flexslider({
						...
						start: function(slider){
							...
							VITHEME.portfolio.portfolioDescMargin();
						},
						after: function(){
		...
		html5Video: function(){
			...
			if( videoEl.length > 0 ) {
				videoEl.each(function(){
					...
					if( VITHEME.isMobile.any() && !element.hasClass('no-placeholder') ) {
						...
						if( $(this).next().is(':hidden') ) {
							...
							$(this).toggleClass('acctitlec').next().stop().slideDown("normal", function(){
								if( ( $body.hasClass('device-sm') || $body.hasClass('device-xs') ) && element.hasClass('scroll-on-open') ) {
									$('html,body').stop(true).animate({
										'scrollTop': clickTarget.offset().top - ( VITHEME.initialize.topScrollOffset() - 40 )
									}, 800, 'easeOutQuad' );
								}
							});
						}
		...
		counter: function(){
			...
			if( $counterEl.length > 0 ){
				$counterEl.each(function(){
					...
					if( $body.hasClass('device-xl') || $body.hasClass('device-lg') ){
						element.appear( function(){
							VITHEME.widget.runCounter( element, counterElementComma );
						},{accX: 0, accY: -120},'easeInCubic');
					} else {
						VITHEME.widget.runCounter( element, counterElementComma );
					}
					...
					if( $body.hasClass('device-xl') || $body.hasClass('device-lg') ){
						...
						element.appear( function(){
							if (!element.hasClass('skills-animated')) {
								...
								VITHEME.widget.runRoundedSkills( element, properties );
							...
							element.find(".slider-wrap").html(sm_format_twitter3(tweets)).promise().done( function(){
								var timer = setInterval(function(){
									if( element.find('.slide').length > 1 ) {
										...
										var t = setTimeout( function(){ VITHEME.widget.loadFlexSlider(); }, 500);
										clearInterval(timer);
									}
								},500);
							});
		...
		masonryThumbs: function(){
			...
			if( $masonryThumbsEl.length > 0 ){
				$masonryThumbsEl.each( function(){
					...
					VITHEME.widget.masonryThumbsArrange( masonryItemContainer );
				});
			}
		},
		...
		linkScroll: function(){
			$("a[data-scrollto]").off( 'click' ).on( 'click', function(){
				...
				if( !divScrollOffset ) { divScrollOffset = VITHEME.initialize.topScrollOffset(); }
						...
						$(form).ajaxSubmit({
							...
							success: function( data ) {
								...
								if( elementAlert == 'inline' ) {
									...
									VITHEME.widget.notifications( elementResult );
								}
	...
	VITHEME.isMobile = {
		...
		any: function() {
			return (VITHEME.isMobile.Android() || VITHEME.isMobile.BlackBerry() || VITHEME.isMobile.iOS() || VITHEME.isMobile.Opera() || VITHEME.isMobile.Windows());
		}
	};

	VITHEME.documentOnResize = {

		init: function(){

			var t = setTimeout( function(){
				VITHEME.header.topsocial();
				VITHEME.header.fullWidthMenu();
				VITHEME.header.overlayMenu();
				VITHEME.initialize.fullScreen();
				VITHEME.initialize.dataResponsiveHeights();
				VITHEME.initialize.verticalMiddle();
				VITHEME.initialize.testimonialsGrid();
				VITHEME.initialize.stickFooterOnSmall();
				VITHEME.initialize.stickyFooter();
				VITHEME.slider.sliderParallaxDimensions();
				VITHEME.slider.captionPosition();
				VITHEME.portfolio.arrange();
				VITHEME.portfolio.portfolioDescMargin();
				VITHEME.widget.tabsResponsiveResize();
				VITHEME.widget.tabsJustify();
				VITHEME.widget.html5Video();
				VITHEME.widget.masonryThumbs();
				VITHEME.initialize.dataResponsiveClasses();
	...
	VITHEME.documentOnReady = {

		init: function(){
			VITHEME.initialize.init();
			VITHEME.header.init();
			if( $slider.length > 0 || $sliderElement.length > 0 ) { VITHEME.slider.init(); }
			if( $portfolio.length > 0 ) { VITHEME.portfolio.init(); }
			VITHEME.widget.init();
			VITHEME.documentOnReady.windowscroll();
		},
		...
		windowscroll: function(){
			...
			VITHEME.header.stickyMenu( headerWrapOffset );
			VITHEME.header.stickyPageMenu( pageMenuOffset );

			$window.on( 'scroll', function(){

				VITHEME.initialize.goToTopScroll();
	...
	VITHEME.documentOnLoad = {

		init: function(){
			VITHEME.slider.captionPosition();
			VITHEME.slider.swiperSliderMenu(true);
			VITHEME.slider.revolutionSliderMenu(true);
			VITHEME.initialize.testimonialsGrid();
			VITHEME.initialize.verticalMiddle();
			VITHEME.initialize.stickFooterOnSmall();
			VITHEME.initialize.stickyFooter();
			VITHEME.portfolio.gridInit( $gridContainer );
			VITHEME.portfolio.filterInit();
			VITHEME.portfolio.shuffleInit();
			VITHEME.portfolio.arrange();
			VITHEME.portfolio.portfolioDescMargin();
			VITHEME.widget.parallax();
			VITHEME.widget.loadFlexSlider();
			VITHEME.widget.html5Video();
			VITHEME.widget.masonryThumbs();
			VITHEME.header.topsocial();
			VITHEME.header.responsiveMenuClass();
			VITHEME.initialize.modal();
		}
	...
	$(document).ready( VITHEME.documentOnReady.init );
	$window.on( 'load', VITHEME.documentOnLoad.init );
	$window.on( 'resize', VITHEME.documentOnResize.init );
})(jQuery);
